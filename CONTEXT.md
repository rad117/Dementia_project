...
- **Code:** Backend Phase 1 done — `backend/` now persists assessments in SQLite and serves the 3-step contract the frontend expects (`POST /assessments` → `POST /assessments/{id}/audio` → `GET /assessments/{id}/results`), backed by a real trained model (see `ml/`); the legacy mock endpoint (`POST /api/assessments`) was removed in a backend cleanup pass (2026-09-20) now that the real contract is proven end-to-end. ML pipeline (`ml/`): **two trained baselines now exist** — `models/baseline_v1/` (acoustics-only, Logistic Regression + Random Forest) and `models/baseline_v2/` (acoustic + ASR + NLP fused, same two model types, trained via `python -m ml.training.train_baseline --use-asr-nlp-features --model-dir models/baseline_v2 --model-version-tag v2-fused`). Both committed to the repo (small, no git-lfs needed — see `models/README.md`). **Fusion improved cross-validated ROC-AUC substantially** (RF: 0.605→0.791, LogReg: 0.668→0.778) on this small (N=164) dataset — encouraging but not a claim of clinical validity at this sample size. `ml/inference/predict.py` is feature-name-aware: it only invokes ASR/NLP (and their per-request latency cost) when the loaded model's `feature_names` require it, so `models/baseline_v1` stays fast. **Decision made (2026-09-20): the backend's live default (`ml/inference/predict.py`'s `model_dir` default, and hence what `backend/routes/assessments.py` actually serves) is now `models/baseline_v2`**, accepting the added per-request ASR latency (seconds, not milliseconds) for the cross-validated ROC-AUC gain. `models/baseline_v1` remains available as a fast acoustics-only fallback via an explicit `model_dir` argument.
- **Next concrete action:** get demographic metadata and storage/consent terms from ma'am (Section 12 — still open, lower urgency than language/task which are now resolved). The `needs_clinician_review` threshold is no longer a bare placeholder — see Section 9 Phase 1 for the Youden's-index method now in use (2026-09-20); it's a statistically principled choice from a small sample, not a clinically validated cutoff, and should still be revisited once real clinical evaluation is possible.
...
- [x] Build a mock ML inference endpoint returning structured JSON (see Section 10 for shape). — Originally `POST /api/assessments` in `backend/routes/assessments.py` with mock logic in `backend/services/mock_inference.py`; removed 2026-09-20 once the real 3-step contract (Section 1) was proven and no longer needed a comparison harness. Full setup/run/test instructions in `backend/README.md`.
...
- [x] Train first interpretable baseline (Logistic Regression / Random Forest), evaluate with full metric set (Section 8), replacing the mock inference endpoint. — `ml/training/train_baseline.py`, artifacts in `models/baseline_v1/` (acoustics-only) and `models/baseline_v2/` (acoustic+ASR+NLP fused, `--use-asr-nlp-features`) (both gitignored); wired into the backend's real endpoints via `ml/inference/predict.py` (Section 10), which is feature-name-aware and only pays the ASR-latency cost for a model that needs it. Backend's live default is now `baseline_v2` (promoted 2026-09-20, Section 1). Each model's `metadata.json` now also carries a `review_threshold` (2026-09-20) computed via Youden's index (max of sensitivity+specificity-1) on the held-out test split — `ml/inference/predict.py` reads it per-model instead of a hardcoded constant. This is a statistically principled cutoff from a small (N=164) sample, not a clinically validated one; published instrument cutoffs (MMSE, MoCA) don't transfer since they're on different scales entirely.
